...
const DEFAULTS = Object.freeze({
  ...
  visualReviewManifest: 'data/manifests/9702_visual_review_2026_06_09_manifest_v1.json',
  plainTextV1Json: 'docs/reports/2026-06-10-9702-question-plain-text-v1.json',
  plainTextV2Json: 'docs/reports/2026-06-10-9702-question-plain-text-v2.json',
  authorityJson: 'docs/reports/2026-06-10-9702-authority-alignment.json',
  consumptionJson: 'docs/reports/2026-06-10-9702-question-plain-text-v2-consumption.json',
  jsonOut: 'docs/reports/2026-06-10-9702-evidence-layers-gate.json',
  markdownOut: 'docs/reports/2026-06-10-9702-evidence-layers-gate.md',
...
function printUsage() {
  writeStdoutLine([
    'Usage: node scripts/learning/build_9702_evidence_layers_gate.js',
...
export function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--visual-review-manifest') {
      options.visualReviewManifest = requiredValue(argv, index, token);
      index += 1;
      continue;
    }
...
function manifestIdFromPath(repoPath) {
  return path.basename(repoPath).replace(/_page_chain_surface_v1\.json$/, '');
}
...
function resolveSurfaceIndex(surface, selectedItem) {
  if (Number.isInteger(selectedItem.source_manifest_index)) {
    return selectedItem.source_manifest_index;
  }
  return normalizeArray(surface.items).findIndex((item) => item?.storage_key === selectedItem.storage_key);
}

function loadAcceptedRows(rootDir, visualReviewManifest, blockers) {
  const visualManifest = readJson(rootDir, visualReviewManifest);
  const surfaceRecordsByPath = new Map();
  const rows = [];

  for (const selectedItem of normalizeArray(visualManifest.items)) {
    const sourceManifest = selectedItem.source_manifest || selectedItem.source_locator_surface_manifest_path;
    if (!sourceManifest) {
      addBlocker(blockers, 'visual_review_row_missing_source_manifest', selectedItem.storage_key);
      continue;
    }
    if (!surfaceRecordsByPath.has(sourceManifest)) {
      const surface = readJson(rootDir, sourceManifest);
      surfaceRecordsByPath.set(sourceManifest, {
        path: sourceManifest,
        payload: surface,
        items: normalizeArray(surface.items),
        selected_count: 0,
      });
    }
    const record = surfaceRecordsByPath.get(sourceManifest);
    const sourceManifestIndex = resolveSurfaceIndex(record.payload, selectedItem);
    const surfaceItem = sourceManifestIndex >= 0 ? record.items[sourceManifestIndex] || null : null;
    if (!surfaceItem) {
      addBlocker(blockers, 'visual_review_row_missing_source_surface_item', selectedItem.storage_key, {
    ...
    rows.push({
      ...
      shard_id: manifestIdFromPath(sourceManifest),
    });
  }

  return {
    visualManifest,
    surfaceRecords: [...surfaceRecordsByPath.values()].sort((left, right) => left.path.localeCompare(right.path)),
    rows,
  };
}
...
function buildRowEvidence({
  ...
  if (authority?.canonical_syllabus_detailed_topic_claimed === true) {
    addBlocker(blockers, 'canonical_syllabus_detailed_topic_claimed', storageKey);
  }
...
function validateVisualGate({ blockers, expectedRows, visualGate, visualManifest }) {
  const counts = visualGate.counts || {};
  if (
    visualGate.gate_status !== 'passed'
    || Number(counts.selected_rows || 0) !== expectedRows
    || Number(counts.accepted_rows || 0) !== expectedRows
    || Number(counts.rejected_rows || 0) !== 0
    || Number(counts.missing_review_rows || 0) !== 0
    || Number(counts.blocker_count || 0) !== 0
    || visualGate.production_ready_claimed === true
  ) {
    addBlocker(blockers, 'visual_gate_not_pass_for_exact_9702_scope', null, {
      gate_status: visualGate.gate_status || null,
      counts,
      production_ready_claimed: visualGate.production_ready_claimed ?? null,
    });
  }
  if (Number(visualManifest.item_count || normalizeArray(visualManifest.items).length) !== expectedRows) {
    addBlocker(blockers, 'visual_review_manifest_count_mismatch', null, {
      expected_rows: expectedRows,
      actual_rows: visualManifest.item_count ?? normalizeArray(visualManifest.items).length,
    });
  }
}
...
function validateAggregateArtifacts({
  ...
  validateVisualGate({ blockers, expectedRows, visualGate, visualManifest });
  ...
  if (Number(v2.summary?.text_only_ready_rows || 0) + Number(v2.summary?.image_context_required_rows || 0) !== expectedRows) {
    addBlocker(blockers, 'text_ready_split_mismatch', null, {
      expected_rows: expectedRows,
      text_only_ready_rows: v2.summary?.text_only_ready_rows ?? null,
      image_context_required_rows: v2.summary?.image_context_required_rows ?? null,
    });
  }
  if (Number(v2.summary?.missing_normalized_plain_text || 0) !== 0) {
    addBlocker(blockers, 'missing_normalized_plain_text_rows_not_zero', null, {
      actual_rows: v2.summary?.missing_normalized_plain_text,
    });
  }
  if (Number(authority.summary?.canonical_syllabus_detailed_topic_claimed_rows || 0) !== 0) {
    addBlocker(blockers, 'canonical_syllabus_detailed_topic_claimed_rows_not_zero', null, {
      actual_rows: authority.summary?.canonical_syllabus_detailed_topic_claimed_rows,
    });
  }
...
function summarizeShard({ shardId, rows, generatedOn, reportsDir }) {
  ...
  return {
    schema_version: '9702_evidence_layers_shard_closeout_v1',
    ...
    summary: {
      ...
      authority_component_aligned_rows: countWhere(rows, (row) => row.authority_alignment_status === 'component_aligned_topic_hint_recorded'),
...
export function build9702EvidenceLayersGate({
  rootDir = ROOT,
  generatedOn = DEFAULTS.generatedOn,
  visualGateJson = DEFAULTS.visualGateJson,
  visualReviewManifest = DEFAULTS.visualReviewManifest,
  ...
  const { visualManifest, surfaceRecords, rows: surfaceRows } = loadAcceptedRows(rootDir, visualReviewManifest, blockers);
  const expectedRows = surfaceRows.length;
  ...
  const updatedSurfaces = surfaceRecords.map((record) => {
    const selectedRows = record.items.filter((item) => rowsByStorageKey.has(item.storage_key));
  ...
  return {
    schema_version: '9702_evidence_layers_gate_v1',
    generated_on: generatedOn,
    subject_code: '9702',
    status,
    verdict: status === 'pass' ? '9702-evidence-layers-ready-local-consumption-proven' : '9702-evidence-layers-blocked',
    production_ready_claimed: false,
    boundary: {
    ...
    summary: {
      selected_surface_manifests: surfaceRecords.length,
...
function renderShardMarkdown(shard) {
  ...
  const blockerLines = shard.blockers.length
    ...
    : '- none';
  return [
    `# 9702 ${shard.shard_id} Evidence Layers Closeout`,
    '',
    `- status: \`${shard.status}\``,
...
export function render9702EvidenceLayersMarkdown(result) {
...
export function write9702EvidenceLayersArtifacts(result, {
  rootDir = ROOT,
  jsonOut = DEFAULTS.jsonOut,
  markdownOut = DEFAULTS.markdownOut,
} = {}) {
  ...
  writeText(rootDir, markdownOut, render9702EvidenceLayersMarkdown(result));
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  const result = build9702EvidenceLayersGate(options);
  write9702EvidenceLayersArtifacts(result, {
  ...
  if (result.status !== 'pass') {
    throw new Error('9702 evidence layers gate failed.');
  }
  return 0;
}
