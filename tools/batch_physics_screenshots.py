...
def process_paper(paper_dir, out_root, dpi, mode):
    ...
    for pdf_path in pdfs:
        ...
        if mode == "mcq":
            run_script(Path("tools") / "mcq_screenshot.py", pdf_path, out_dir, dpi)
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Batch screenshots for 9702 Physics papers")
    parser.add_argument("--paper", required=True, help="paper1|paper2|paper3|paper4|paper5")
    parser.add_argument("--dpi", type=int, default=400)
    parser.add_argument("--out-root", default="outputs/screenshots/physics")
    args = parser.parse_args()

    paper = args.paper.lower()
    paper_dir = Path("data") / "past-papers" / "9702Physics" / paper
    out_root = Path(args.out_root) / paper

    mode = "mcq" if paper == "paper1" else "structured"
    process_paper(paper_dir, out_root, args.dpi, mode)
