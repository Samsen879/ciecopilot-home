...
CREATE TABLE IF NOT EXISTS public.learning_artifacts (
  ...
  artifact_state TEXT NOT NULL DEFAULT 'unverified' CHECK (artifact_state IN ('unverified', 'verified', 'released', 'contested', 'superseded')),
  ...
  CHECK (artifact_state <> 'verified' OR (verified_by IS NOT NULL AND verified_at IS NOT NULL AND verification_evidence_ref IS NOT NULL)),
  CHECK (artifact_state <> 'released' OR (verified_by IS NOT NULL AND verified_at IS NOT NULL AND verification_evidence_ref IS NOT NULL AND released_by IS NOT NULL AND released_at IS NOT NULL AND release_evidence_ref IS NOT NULL)),
  CHECK (artifact_state <> 'contested' OR (contested_by IS NOT NULL AND contested_at IS NOT NULL AND contested_reason IS NOT NULL)),
  CHECK (artifact_state <> 'superseded' OR superseded_at IS NOT NULL)
);
