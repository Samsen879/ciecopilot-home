...
from scripts.vlm.build_9231_wave4_crop_repair_gate_v1 import (  # noqa: E402
    DEFAULT_WAVE4_REPAIR_SHARD_IDS,
    build_9231_wave4_crop_repair_gate,
...
def _input_manifest_for(pdf_path: Path, shard_id: str) -> dict:
    rows = [
        ...
        {
            "storage_key": "9231/w23_qp_21/questions/q02.png",
            "subject_code": "9231",
            "syllabus_code": "9231",
            "year": 2023,
            "session": "w",
...
def test_default_wave4_repair_shards_are_remaining_clean_crop_incomplete_set():
    assert DEFAULT_WAVE4_REPAIR_SHARD_IDS == (
...
def test_build_9231_wave4_crop_repair_gate_updates_surface_rows_with_wave4_labels(tmp_path: Path):
    shard_id = "9231_p2_w23_standard_001"
    pdf_path = _write_sample_pdf(
        tmp_path / "data" / "past-papers" / "9231Further-Mathematics" / "paper2" / "9231_w23_qp_21.pdf"
    )
    ...
    result = build_9231_wave4_crop_repair_gate(
        wave4_shard_ids=[shard_id],
        manifest_root=tmp_path / "data" / "manifests",
        output_root=tmp_path / "data" / "crops" / "9231-crop-repair-wave4",
        workspace_root=tmp_path,
        generated_on="2026-06-05",
        render_scale=1.0,
    )

    assert result["crop_manifest"]["schema_version"] == "9231_wave4_crop_manifest_v1"
    assert result["crop_manifest"]["scope"]["stage"] == "9231 wave4 crop repair render/crop gate"
    assert result["report"]["schema_version"] == "9231_wave4_crop_repair_gate_v1"
    assert result["report"]["gate_status"] == "wave4_crop_render_complete_pending_visual_review"
    assert result["report"]["summary"]["wave4_shards"] == 1
    ...
    assert result["report"]["wave4_shard_ids"] == [shard_id]
    ...
    assert "wave4_crop_gate" in updated_surface
    assert "pilot_crop_gate" not in updated_surface
    assert updated_surface["surface_status"] == "locator_rows_ready_pending_visual_review"
    assert first["crop_status"] == "complete"
    assert first["surface_evidence_status"] == "local_wave4_crop_render_complete_pending_visual_review"
    assert first["page_chain_surface_status"] == "wave4_crop_render_complete_pending_visual_review"
    assert first["production_ready_claimed"] is False
    assert first["db_consumption_claimed"] is False
    assert first["search_consumption_claimed"] is False
    assert first["rag_consumption_claimed"] is False
    assert first["external_vlm_or_api_used"] is False
    assert (tmp_path / first["crop_paths"][0]).exists()


def test_write_9231_wave4_crop_repair_gate_outputs_persists_reports_and_surfaces(tmp_path: Path):
    shard_id = "9231_p2_w23_standard_001"
    pdf_path = _write_sample_pdf(
        tmp_path / "data" / "past-papers" / "9231Further-Mathematics" / "paper2" / "9231_w23_qp_21.pdf"
    )
    ...
    paths = write_9231_wave4_crop_repair_gate_outputs(
        result,
        crop_manifest_path=tmp_path / "data" / "manifests" / "9231_wave4_crop_repair_2026_06_05_crop_manifest_v1.json",
        report_json_path=tmp_path / "docs" / "reports" / "2026-06-05-9231-wave4-crop-repair-gate.json",
        report_md_path=tmp_path / "docs" / "reports" / "2026-06-05-9231-wave4-crop-repair-gate.md",
    )
    ...
    assert persisted_surface["items"][0]["page_chain_surface_status"] == "wave4_crop_render_complete_pending_visual_review"
