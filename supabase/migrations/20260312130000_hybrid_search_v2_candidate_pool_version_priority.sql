-- Add stable-identity version priority inside dense/key candidate pools.
-- Keep RRF fusion intact, but ensure older corpus versions do not crowd newer
-- versions out of the semantic or lexical candidate pools before fusion.
...
BEGIN
  ...
  dense AS (
    SELECT
      id,
      ROW_NUMBER() OVER (
        ORDER BY sem_distance ASC, id ASC
      )::int AS r_sem
    FROM dense_unique
    ORDER BY sem_distance ASC, id ASC
    LIMIT p_dense_pool
  ...
  key AS (
    SELECT
      id,
      ROW_NUMBER() OVER (
        ORDER BY key_score DESC, id ASC
      )::int AS r_key
    FROM key_unique
    ORDER BY key_score DESC, id ASC
    LIMIT p_key_pool
