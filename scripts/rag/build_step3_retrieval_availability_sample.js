...
function selectCases(candidates, limit) {
  const selected = [];
  const used = new Set();
...
function main() {
  ...
  const outDataset = path.join(ROOT, argv['out-dataset'] || `data/eval/rag_step3_retrieval_availability_sample_${subjectCode}.json`);
  const outManifest = path.join(ROOT, argv['out-manifest'] || `data/eval/rag_step3_retrieval_availability_sample_${subjectCode}_manifest.json`);
  ...
  const selected = selectCases(source, limit);
  const manifest = {
