...
|:---:|---|:---:|
| 1 | 收敛默认入口和目录拓扑 | ✅ 已完成 |
| 2 | 收敛 truth / diagnose / decide 闭环 | ✅ 已完成 |
| 3 | 补恢复和接力闭环 | ✅ 已完成 |
| 4 | 推进 assist execution | ✅ 已完成 |
| 5 | 打通任务状态和 closeout | ✅ 已完成 |
| 6 | 历史债务清理和 soak | ✅ 已完成 |
...
这份路线图启动时，它离“完全态”还差一段距离；截至本次更新，这些差距已经按 phase 1-6 收敛完成。

### 当前最真实的状态

AO 现在更像：

- 一个已经完成这份路线图 6 个阶段收口、进入长期运行与归档策略阶段的 repo-local 工程控制面

当前仍值得继续投入的方向是：

- retired durable history 的归档/压缩
- repo knowledge profile 的持续维护
- 更长周期的生产化 soak 和更广的 incident corpus
...
- ~~第四阶段 assist execution：`4-6` 个工作日~~ ✅
- ~~第五阶段任务状态与 closeout：`4-6` 个工作日~~ ✅
- ~~第六阶段历史债务清理和 soak：`5-8` 个工作日~~ ✅

合计建议预期：

- ~~**约 4-7 周**~~ Phase 1-6 已完成
...
这是 phase 1-6 启动前最关键的一节。

以下差距分析保留为 phase 1-6 启动前的历史判断，用来解释路线图为什么按这个顺序展开；它们不再代表当前未完成项。

不要把这部分历史上下文和今天的完成状态混读。

阶段启动时 AO 主要缺的，主要不是更多命令，而是 **闭环能力**。

### 3.1 默认入口还带临时桥接

阶段启动时，主线里的 `agent-orchestrator.yaml` 仍然使用了临时桥接路径，把项目根指到干净 baseline worktree。
...
也就是说，当时 AO 已经能“看懂很多问题”，但还没有把“恢复问题”完全吃下来。

### 3.3 lifecycle 当时还是 decide-only
...
当时从代码上看，action executor 已经有雏形，而且部分 `class_a` 动作允许执行。
...
### 3.7 长期运行和 incident 当时还没经过足够 soak
...
- 已补 `ao-state` debt inventory，并明确 `keep_evidence / archive_candidate / cleanup_candidate` contract
- 已完成一次非破坏式历史 artifact 清理，并把 loose phase outputs 迁入 `runs/phase6/`
- 已完成 smoke / incident drills / eval baseline 对比
- 已完成一次真实 managed-task soak：`issue-9606` 经过 `enroll -> pause -> adopt -> retire`，随后清理了 synthetic branch/worktree
- 清理后的 `ao-state` 仅保留 `managed_task issue-9606` 的 durable retired history 作为 `archive_candidate`，不再保留 synthetic cleanup candidates
...
如果现在继续推进，不再是 phase 1-6 的收尾问题，而是要决定下一轮路线是否进入：

- retired durable history 的归档/压缩操作
- repo knowledge profile 的补齐
- 更长周期的生产化 soak 和更广的 incident corpus
- 超出当前红线之外的新自治边界
...
AO 现在已经完成这份路线图里的 6 个阶段，具备了“控制面骨架 + 可控执行边界 + 任务/closeout 收口 + phase-6 debt/soak 证据”。

下一步不再是补齐这份 roadmap，而是决定下一轮更高阶的长期运行和归档策略。
