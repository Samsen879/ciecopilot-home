...
- `2026-04-15-question-search-slice-v1-report.md` - stage-3 structured question-search closeout report with a manual verification supplement plus runner-emitted gate output covering descriptor-surface counts for `v0`/`v1`/`prod_v1`, the frozen fallback contract, threshold results, and the explicit fail posture for the current paper-backed data gap.
- `2026-04-15-9709-question-bank-data-recovery-report.md` - issue `#202` recovery report covering deterministic pilot curriculum-node seeding, manifest-backed `paper_question` inserts, before/after `imported_question` vs `paper_question` counts, and the still-red descriptor posture.
- `2026-04-16-9709-qwen-wave1-execution-report.md` - issue `#201` real pilot execution report for the deterministic `9709` wave-1 slice, including checked-in lane results, lane-aware QC counts, spot-check evidence, and the explicit not-ready descriptor posture.
- `2026-04-16-9709-question-search-wave1-closeout-report.md` - issue `#203` closeout report rerunning the focused question-search verification matrix and the live `9709` gate, showing the recovered pilot still fails because paper-backed projection rows are present but not hydrated with paper metadata or descriptor text.
- `2026-04-16-9709-question-search-gate-baseline-report.md` - durable checked-in gate rerun evidence for issue `#198`, capturing the still-red `9709` baseline against `data/eval/question_search_gold_9709_v1.json`.
