...
function analyzeConflictSignals(item) {
  ...
  return {
    id: item?.id || null,
    anchor: buildConflictAnchor(item),
    normalized,
...
function buildConflictAnchor(item) {
  const sourceRef = item?.source_ref;
  const assetId = String(sourceRef?.asset_id || '').trim();
  const questionId = String(sourceRef?.question_id || sourceRef?.parent_question_id || '').trim().toLowerCase();
  if (!assetId || !questionId) return null;
...
function hasConflict(evidence) {
  ...
  for (let index = 0; index < signals.length; index += 1) {
    for (let otherIndex = index + 1; otherIndex < signals.length; otherIndex += 1) {
      ...
      if (left.anchor && right.anchor && left.anchor !== right.anchor) continue;
