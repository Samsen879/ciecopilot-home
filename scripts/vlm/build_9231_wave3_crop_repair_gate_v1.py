#!/usr/bin/env python3
"""Build local-only render/crop evidence for 9231 wave3 crop repair shards."""
...
SCHEMA_VERSION = "9231_wave3_crop_repair_gate_v1"
CROP_MANIFEST_SCHEMA_VERSION = "9231_wave3_crop_manifest_v1"
DEFAULT_WAVE3_REPAIR_SHARD_IDS = (
    "9231_p3_s21_standard_001",
    "9231_p3_s22_standard_001",
    "9231_p3_s23_standard_001",
    "9231_p3_w20_standard_001",
    "9231_p3_w21_standard_001",
    "9231_p3_w22_standard_001",
    "9231_p3_w23_standard_001",
    "9231_p3_w24_standard_001",
)
SELECTION_STRATEGY = (
    {
        "bucket": "Paper 3 summer 2021 to winter 2024",
        "shard_ids": DEFAULT_WAVE3_REPAIR_SHARD_IDS,
    },
)
...
def _replace_wave3_strings(value: Any) -> Any:
    if isinstance(value, str):
        return (
            value.replace("local_pilot_crop_render", "local_wave3_crop_render")
            .replace("pilot_crop_render", "wave3_crop_render")
            .replace("9231 pilot shard", "9231 wave3 shard")
            .replace("9231 pilot shards", "9231 wave3 shards")
            .replace("representative 9231 pilot", "representative 9231 wave3")
            .replace("pilot page-chain", "wave3 page-chain")
        )
    if isinstance(value, list):
        return [_replace_wave3_strings(item) for item in value]
    if isinstance(value, dict):
        return {key: _replace_wave3_strings(item) for key, item in value.items()}
    return value


def _wave3_crop_manifest(crop_manifest: dict[str, Any], *, generated_on: str) -> dict[str, Any]:
    updated = _replace_wave3_strings(crop_manifest)
    updated["schema_version"] = CROP_MANIFEST_SCHEMA_VERSION
    updated["manifest_id"] = f"9231_wave3_crop_repair_{generated_on.replace('-', '_')}_crop_manifest_v1"
    scope = dict(updated.get("scope") or {})
    scope.update(
        {
            "stage": "9231 wave3 crop repair render/crop gate",
            "source_scope_label": "8 Paper 3 standard 9231 wave3 input manifests",
            "surface_update": "wave3 page-chain surface manifests receive local crop/render references only",
...
def _wave3_surface_record(record: dict[str, Any], *, generated_on: str) -> dict[str, Any]:
    payload = _replace_wave3_strings(record["payload"])
    pilot_gate = payload.pop("pilot_crop_gate", {})
    payload["wave3_crop_gate"] = {
        **pilot_gate,
    ...
    # Mark the surface_status as ready for visual review
    payload["surface_status"] = "locator_rows_ready_pending_visual_review"
    for item in payload.get("items") or []:
        if isinstance(item, dict):
            item["surface_evidence_status"] = "local_wave3_crop_render_complete_pending_visual_review"
            item["page_chain_surface_status"] = "wave3_crop_render_complete_pending_visual_review"
    return {
        **record,
        "payload": payload,
    }
...
def _wave3_report(
    report: dict[str, Any],
    *,
    wave3_shard_ids: list[str],
    generated_on: str,
    source_cleanliness_risks: dict[str, Any],
) -> dict[str, Any]:
    updated = _replace_wave3_strings(report)
    ...
    summary["wave3_shards"] = summary.pop("pilot_shards", len(wave3_shard_ids))
    updated["summary"] = summary
    updated.pop("pilot_shard_ids", None)
    updated["wave3_shard_ids"] = wave3_shard_ids
...
def render_9231_wave3_crop_repair_gate_markdown(
    report: dict[str, Any],
    *,
    ...
    lines = [
        ...
        "",
        (
            "Conclusion: 8 representative 9231 wave3 crop repair shards have local deterministic render/crop assets and surface crop references, pending visual review, OCR/text evidence, v1/v2 text, and normalized_plain_text consumption gates."
            if report["gate_status"] == "wave3_crop_render_complete_pending_visual_review"
            else "Conclusion: 9231 wave3 crop repair gate is blocked and needs local locator/crop fixes before broader rollout."
        ),
        "",
        "## Selection Strategy",
        "",
    ...
    lines.extend(
        [
            ...
            "| metric | value |",
            "| --- | ---: |",
            f"| wave3 shards | {summary['wave3_shards']} |",
    ...
    lines.extend(
        [
            ...
            "",
            "- Spot-check representative wave3 crop images visually before broad 9231 rollout.",
...
def build_9231_wave3_crop_repair_gate(
    *,
    wave3_shard_ids: list[str] | tuple[str, ...] = DEFAULT_WAVE3_REPAIR_SHARD_IDS,
    manifest_root: str | Path = "data/manifests",
    output_root: str | Path = "data/crops/9231-crop-repair-wave3",
    ...
    wave3_ids = [str(value) for value in wave3_shard_ids]
    result = build_9231_pilot_shard_crop_gate(
        pilot_shard_ids=wave3_ids,
    ...
    updated_surfaces = [
        _wave3_surface_record(record, generated_on=generated)
        for record in result["updated_surface_manifests"]
    ]
    crop_manifest = _wave3_crop_manifest(result["crop_manifest"], generated_on=generated)
    return {
        "crop_manifest": crop_manifest,
        "report": _wave3_report(
            result["report"],
            wave3_shard_ids=wave3_ids,
            generated_on=generated,
...
def write_9231_wave3_crop_repair_gate_outputs(
    result: dict[str, Any],
    *,
    crop_manifest_path: str | Path,
    ...
    report_md_path.write_text(
        render_9231_wave3_crop_repair_gate_markdown(
            result["report"],
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build local-only 9231 wave3 crop repair gate artifacts.")
    parser.add_argument("--wave3-shard-id", action="append", default=[])
    parser.add_argument("--manifest-root", type=Path, default=Path("data/manifests"))
    parser.add_argument("--output-root", type=Path, default=Path("data/crops/9231-crop-repair-wave3"))
    parser.add_argument(
        "--crop-manifest",
        type=Path,
        default=Path("data/manifests/9231_wave3_crop_repair_2026_06_05_crop_manifest_v1.json"),
    )
    parser.add_argument(
        "--report-json",
        type=Path,
        default=Path("docs/reports/2026-06-05-9231-wave3-crop-repair-gate.json"),
    )
    parser.add_argument(
        "--report-md",
        type=Path,
        default=Path("docs/reports/2026-06-05-9231-wave3-crop-repair-gate.md"),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    wave3_ids = args.wave3_shard_id or list(DEFAULT_WAVE3_REPAIR_SHARD_IDS)
    result = build_9231_wave3_crop_repair_gate(
        wave3_shard_ids=wave3_ids,
    ...
    write_9231_wave3_crop_repair_gate_outputs(
        result,
    ...
    return 0 if result["report"]["gate_status"] == "wave3_crop_render_complete_pending_visual_review" else 1
