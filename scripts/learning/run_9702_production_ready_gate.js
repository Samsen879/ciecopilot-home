...
const QUESTION_TYPE_ID = '9702.question_text_foundation.row_surface';
const QUESTION_FAMILY_ID = '9702.question_text_foundation';
const CURRICULUM_VERSION_TAG = '2025-2027_v1';
const RAG_CORPUS_VERSION = 'rag_step3_9702_question_plain_text_v2_phase7_v1';
const FALLBACK_GENERATED_ON = '2026-06-10';
const DEFAULT_EVIDENCE_GATE = 'docs/reports/2026-06-10-9702-evidence-layers-gate.json';

const DEFAULTS = Object.freeze({
  batchId: 'all',
  generatedOn: null,
  evidenceGate: DEFAULT_EVIDENCE_GATE,
...
export const REQUIRED_9702_PRODUCTION_DB_ZERO_FIELDS = Object.freeze([
  'missing_registry',
  'prompt_missing',
  'provenance_missing',
...
function printUsage() {
  writeStdoutLine([
    'Usage: node scripts/learning/run_9702_production_ready_gate.js',
    '  [--batch-id <all|p1_s_historical_001|...>]',
...
export function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--evidence-gate') {
      options.evidenceGate = requiredValue(argv, index, token);
      index += 1;
      continue;
    }
...
function normalizeBatchId(value) {
  return normalizeString(value).replace(/^9702_/, '');
}

function generatedOnToken(generatedOn) {
  return generatedOn.replaceAll('-', '_');
}

function toReportSlug(batchId) {
  return batchId.replaceAll('_', '-');
}

function sourceManifestFromShard(shard = {}) {
  const rows = Array.isArray(shard.rows) ? shard.rows : [];
  const manifests = new Set();
  for (const row of rows) {
    const sourceManifest = normalizeString(row.source_manifest)
      || normalizeString(row.source_surface_manifest);
    if (sourceManifest) {
      manifests.add(sourceManifest);
    }
  }
  return [...manifests].sort();
}

function duplicateIdentityCount(rows = []) {
  const counts = new Map();
  for (const row of rows) {
    const storageKey = normalizeString(row?.storage_key);
    const qNumber = Number(row?.q_number);
    if (!storageKey || !Number.isInteger(qNumber)) {
      continue;
    }
    const key = `${storageKey}#${qNumber}`;
    counts.set(key, (counts.get(key) ?? 0) + 1);
  }
  return [...counts.values()].filter((count) => count > 1).reduce((sum, count) => sum + count, 0);
}

export function build9702ProductionBatchPlan({
  evidenceGate,
  generatedOn = null,
  reportsDir = DEFAULTS.reportsDir,
} = {}) {
  if (!evidenceGate) {
    throw new Error('build9702ProductionBatchPlan requires a Phase 6 evidence gate payload.');
  }
  const effectiveGeneratedOn = generatedOn ?? evidenceGate.generated_on ?? FALLBACK_GENERATED_ON;
  const artifacts = evidenceGate.artifacts ?? {};
  const seen = new Set();
  const blockers = [];
  const batches = (Array.isArray(evidenceGate.shards) ? evidenceGate.shards : [])
    .map((shard) => {
      const shardId = normalizeString(shard.shard_id);
      const batchId = normalizeBatchId(shardId);
      const rows = Array.isArray(shard.rows) ? shard.rows : [];
      const expectedRows = Number(shard.summary?.rows ?? rows.length);
      const shardBlockers = Number(shard.summary?.blockers ?? (Array.isArray(shard.blockers) ? shard.blockers.length : 0));
      const duplicateRows = duplicateIdentityCount(rows);
      const sourceSurfaceManifests = sourceManifestFromShard(shard);
      const batch = {
        batch_id: batchId,
        shard_id: shardId,
        report_slug: toReportSlug(batchId),
        manifest_slug: batchId,
        log_prefix: `9702_${batchId}`,
        label: shardId,
        production_wave: `9702_${batchId}`,
        expected_rows: expectedRows,
        text_only_ready_rows: Number(shard.summary?.text_only_ready_rows ?? 0),
        image_context_required_rows: Number(shard.summary?.image_context_required_rows ?? 0),
        source_surface_manifests: sourceSurfaceManifests,
        ready_manifest: `data/manifests/9702_${batchId}_production_surface_${generatedOnToken(effectiveGeneratedOn)}_manifest_v1.json`,
        artifact_paths: {
          batch_plan: `data/manifests/9702_production_batch_plan_${generatedOnToken(effectiveGeneratedOn)}_manifest_v1.json`,
          ready_manifest: `data/manifests/9702_${batchId}_production_surface_${generatedOnToken(effectiveGeneratedOn)}_manifest_v1.json`,
          db_coverage: `${reportsDir}/${effectiveGeneratedOn}-9702-${toReportSlug(batchId)}-db-coverage-gate.json`,
          production_search: `${reportsDir}/${effectiveGeneratedOn}-9702-${toReportSlug(batchId)}-production-search-gate.json`,
          production_read_model: `${reportsDir}/${effectiveGeneratedOn}-9702-${toReportSlug(batchId)}-production-read-model-gate.json`,
          production_rag: `${reportsDir}/${effectiveGeneratedOn}-9702-${toReportSlug(batchId)}-production-rag-consumption-gate.json`,
          production_ready_gate: `${reportsDir}/${effectiveGeneratedOn}-9702-${toReportSlug(batchId)}-production-ready-gate.json`,
          production_ready_closeout: `${reportsDir}/${effectiveGeneratedOn}-9702-${toReportSlug(batchId)}-production-ready-closeout.md`,
        },
      };

      if (!shardId || !batchId) {
        blockers.push({ check: 'missing_shard_batch_id', shard_id: shardId || null });
      }
      if (seen.has(batchId)) {
        blockers.push({ check: 'duplicate_batch_id', batch_id: batchId });
      }
      seen.add(batchId);
      if (expectedRows !== rows.length) {
        blockers.push({
          check: 'phase6_shard_row_count_mismatch',
          batch_id: batchId,
          expected_rows: expectedRows,
      ...
      if (shardBlockers !== 0) {
        blockers.push({ check: 'phase6_shard_blockers', batch_id: batchId, blockers: shardBlockers });
      }
      if (duplicateRows !== 0) {
        blockers.push({ check: 'phase6_duplicate_storage_key_q_number_rows', batch_id: batchId, duplicate_rows: duplicateRows });
      }
      if (sourceSurfaceManifests.length !== 1) {
        blockers.push({
          check: 'non_deterministic_source_surface_manifest',
          batch_id: batchId,
          source_surface_manifest_count: sourceSurfaceManifests.length,
        });
      }
      return batch;
    })
  ...
  const selectedRows = batches.reduce((sum, batch) => sum + batch.expected_rows, 0);
  if (Number(evidenceGate.summary?.selected_rows) !== selectedRows) {
    blockers.push({
      check: 'phase6_selected_rows_mismatch',
      expected_rows: Number(evidenceGate.summary?.selected_rows ?? 0),
      actual_rows: selectedRows,
    });
  }
  if (Number(evidenceGate.summary?.blockers ?? 0) !== 0) {
    blockers.push({ check: 'phase6_evidence_gate_blockers', blockers: Number(evidenceGate.summary.blockers) });
  }

  return {
    schema_version: '9702_phase7_production_batch_plan_v1',
    generated_on: effectiveGeneratedOn,
    subject_code: '9702',
    status: blockers.length === 0 ? 'ready' : 'blocked',
    production_ready_claimed: false,
    source_evidence_gate: artifacts.json ?? DEFAULT_EVIDENCE_GATE,
    deterministic_strategy: 'one production batch per accepted Phase 6 shard id, sorted lexicographically by shard_id',
    summary: {
      selected_rows: selectedRows,
      source_selected_rows: Number(evidenceGate.summary?.selected_rows ?? selectedRows),
    ...
    blockers,
    boundary: [
      'This is a Phase 7 production batch plan only; it does not claim full all-9702 production readiness.',
      'Full all-9702 readiness remains blocked until #410 aggregate closeout passes on main.',
    ],
  };
}

function evidenceGatePayload(input = DEFAULT_EVIDENCE_GATE) {
...
export function build9702ProductionBatchConfig(batchId = DEFAULTS.batchId, overrides = {}) {
  const evidenceGate = evidenceGatePayload(overrides.evidenceGate ?? DEFAULTS.evidenceGate);
  const generatedOn = overrides.generatedOn ?? evidenceGate.generated_on ?? FALLBACK_GENERATED_ON;
  ...
  const base = plan.batches.find((batch) => batch.batch_id === normalizedBatchId || batch.shard_id === batchId);
  if (!base) {
    throw new Error(`Unsupported 9702 production batch id: ${batchId}`);
  }
  return {
    batchId: base.batch_id,
    reportSlug: base.report_slug,
    manifestSlug: base.manifest_slug,
    logPrefix: base.log_prefix,
    label: base.label,
    productionWave: base.production_wave,
    expectedRows: base.expected_rows,
    scopeDescription: `${base.shard_id} / ${base.expected_rows} rows`,
    shardId: base.shard_id,
    sourceSurfaceManifests: base.source_surface_manifests,
    planPath: base.artifact_paths.batch_plan,
    readyManifest: overrides.readyManifest ?? base.ready_manifest,
    generatedOn,
    reportsDir: overrides.reportsDir ?? DEFAULTS.reportsDir,
    artifacts: plan.source_artifacts,
    v2Artifacts: Object.freeze([plan.source_artifacts.plain_text_v2_json]),
    authorityArtifacts: Object.freeze([plan.source_artifacts.authority_json]),
    consumptionArtifacts: Object.freeze([plan.source_artifacts.consumption_json]),
    evidenceGateArtifacts: Object.freeze([plan.source_evidence_gate]),
  };
}
...
function inferShardId(row = {}) {
  ...
  if (explicit) {
    return explicit.startsWith('9702_') ? explicit : `9702_${explicit}`;
  }
  const sourceManifest = normalizeString(row.source_surface_manifest);
  const match = /data\/manifests\/(9702_p[1-5]_[msw]_(?:historical|promoted_public)_001)_page_chain_surface_v1\.json$/.exec(sourceManifest);
  if (match) {
    return match[1];
  }
  return null;
...
function parseShardId(shardId) {
  const match = /^9702_p(?<paper>[1-5])_(?<session>[msw])_(?<sourceType>historical|promoted_public)_001$/.exec(shardId);
  if (!match?.groups) {
    throw new Error(`Unexpected 9702 phase7 shard id: ${shardId}`);
  }
  return {
    paper: Number(match.groups.paper),
    session: match.groups.session,
    source_type: match.groups.sourceType,
  };
}
...
function buildTopicTitle(topicPath) {
  ...
  if (normalized === '9702') {
    return 'Cambridge International AS & A Level Physics 9702';
  }
  const componentMatch = /^9702\.p([1-5])$/.exec(normalized);
  if (componentMatch) {
...
function topicPaper(topicPath) {
  const match = /^9702\.p([1-5])(?:\.|$)/.exec(normalizeString(topicPath));
  return match ? Number(match[1]) : null;
}

function buildTopicNodes(readyRows = []) {
  const paths = new Set(['9702']);
...
export function build9702ReadyRows({
  batchConfig = build9702ProductionBatchConfig(),
  v2Artifacts,
  authorityArtifacts,
  ...
  for (const v2 of v2Items) {
    ...
    const sourceSurfaceManifest = normalizeString(v2.source_surface_manifest);
    ...
    const searchText = normalizeString(consumption?.search?.search_text);
    const searchTextSource = normalizeString(consumption?.search?.search_text_source);
    const promptValue = normalizeString(consumption?.read_model?.prompt_representation?.value);
    const ragContent = normalizeString(consumption?.rag?.content);
    const ragContentSource = normalizeString(consumption?.rag?.content_source);
    const inBatch = shardId === batchConfig.shardId
      || batchConfig.sourceSurfaceManifests.includes(sourceSurfaceManifest);
    ...
    if (
      ...
      || !consumption
      || !normalizedPlainText
      || consumptionText !== normalizedPlainText
      || !shardId
      || !sourceSurfaceManifest
      || searchText !== normalizedPlainText
      || searchTextSource !== NORMALIZED_TEXT_SOURCE
      || promptValue !== normalizedPlainText
      || ragContent !== normalizedPlainText
      || ragContentSource !== NORMALIZED_TEXT_SOURCE
    ) {
      continue;
    }
    const visualReviewArtifact = normalizeNullableString(v2.provenance?.v1_provenance?.visual_review_evidence_path)
      ?? batchConfig.artifacts.visual_gate_json
      ?? null;
    const cropPaths = normalizeArray(v2.crop_paths).length ? normalizeArray(v2.crop_paths) : normalizeArray(v2.image_assets);
    const textConsumptionStatus = normalizeString(consumption.text_consumption_status)
    ...
    readyRows.push({
      ...
      source_version: normalizeNullableString(v2.source_version),
      source_surface_manifest: sourceSurfaceManifest,
      shard_id: shardId,
      ...
      provenance_artifacts: {
        source_pdf: normalizeString(v2.source_pdf),
        source_surface_manifest: sourceSurfaceManifest,
        source_crop_paths: cropPaths,
        visual_review: visualReviewArtifact,
        question_plain_text_v1: normalizeNullableString(v2.source_v1_text_layer),
        question_plain_text_v2: normalizeString(v2.__artifact_path),
        authority_alignment: normalizeNullableString(authority.__artifact_path),
        local_consumption: normalizeNullableString(consumption.__artifact_path),
        evidence_layers_gate: batchConfig.evidenceGateArtifacts[0] ?? null,
      },
      local_consumption_status: textConsumptionStatus,
      local_search_text_source: searchTextSource,
      local_read_model_prompt_source: NORMALIZED_TEXT_SOURCE,
      local_rag_content_source: ragContentSource,
...
export function build9702Phase7ReadyRows({
  batchId = 'p1_m_historical_001',
  evidenceGate = DEFAULT_EVIDENCE_GATE,
  v2Artifact,
  authorityArtifact,
  consumptionArtifact,
} = {}) {
  const batchConfig = build9702ProductionBatchConfig(batchId, { evidenceGate });
  return build9702ReadyRows({
    batchConfig,
...
function buildReadyManifest({
  ...
  const claimed = ready && productionReadyClaimed;
  return {
    schema_version: `9702_${batchConfig.manifestSlug}_production_surface_manifest_v1`,
    generated_on: generatedOn,
    manifest_id: `9702_${batchConfig.manifestSlug}_production_surface_${generatedOn.replaceAll('-', '_')}_manifest_v1`,
    subject_code: '9702',
    wave: batchConfig.batchId,
    status: ready ? 'ready' : 'blocked',
    production_ready_claimed: claimed,
    summary,
    source_artifacts: artifacts,
    rows: readyRows.map((row) => ({
      ...row,
      production_ready_claimed: claimed,
      db_consumption_claimed: claimed,
      search_consumption_claimed: claimed,
      read_model_consumption_claimed: claimed,
      rag_consumption_claimed: claimed,
    })),
  };
}
...
function dbMetricPass(metrics = {}, rows) {
  return (
    ...
    && REQUIRED_9702_PRODUCTION_DB_ZERO_FIELDS.every((field) => metrics[field] === 0)
  );
}
...
export function build9702ProductionReadyAggregate({
  batchConfig = build9702ProductionBatchConfig(),
  generatedOn,
  readyRows,
  expectedRows = null,
  ...
  const normalizedReadyRows = readyRows.map((row) => ({
    ...row,
    shard_id: normalizeString(row.shard_id) || inferShardId(row) || batchConfig.shardId,
  }));
  const rows = normalizedReadyRows.length;
  const requiredRows = Number.isInteger(expectedRows) ? expectedRows : batchConfig.expectedRows;
  const byShard = groupRowsByShard(normalizedReadyRows);
  const blockers = [];
  const gateCounts = {
    ...
    production_read_model:
      ...
      && gateMetricEquals(readModelGate, 'read_model_normalized_text_matches', rows)
      && gateMetricEquals(readModelGate, 'prompt_representation_matches_normalized_plain_text', rows)
        ? 1
        : 0,
    production_rag:
      ragGate?.status === 'pass'
      && gateMetricEquals(ragGate, 'rag_chunks_present', rows)
      ...
      && gateMetricEquals(ragGate, 'rag_chunks_with_fts', rows)
        ? 1
        : 0,
  };
  ...
  return {
    schema_version: `9702_${batchConfig.manifestSlug}_production_ready_gate_v1`,
    generated_on: generatedOn,
    status,
    ...
    scope: {
      ...
      shards: byShard.size,
      rows,
      source_pdfs: new Set(normalizedReadyRows.map((row) => row.source_pdf).filter(Boolean)).size,
      ready_manifest: readyManifestPath,
    },
    summary: {
      selected_rows: rows,
      text_only_ready_rows: normalizedReadyRows.filter((row) => row.text_consumption_status === 'text_only_ready').length,
      image_context_required_rows: normalizedReadyRows.filter((row) => row.text_consumption_status === 'image_context_required').length,
    ...
    required_gate_contract: {
      ...
      db_coverage: {
        ...
        required_zero_fields: REQUIRED_9702_PRODUCTION_DB_ZERO_FIELDS,
      },
...
export function build9702Phase7ProductionReadyAggregate(args = {}) {
  return build9702ProductionReadyAggregate({
    batchConfig: build9702ProductionBatchConfig(args.batchId ?? 'p1_m_historical_001'),
...
VALUES (
  '${QUESTION_FAMILY_ID}',
  '9702',
  '9702 question text foundation',
  'Question-text foundation family for row-level Cambridge 9702 production surfaces.',
  'validated'
)
...
VALUES (
  ...
  '${QUESTION_FAMILY_ID}',
  '9702',
  '9702 question text foundation row',
  'Materialized row-surface classifier for 9702 question text foundation rows; not a released scoring taxonomy.',
  '["subject:9702", "scope:question_text_foundation", "release_scope:non_released_fallback"]'::jsonb,
  'validated'
)
...
SELECT
  ...
  '${QUESTION_TYPE_ID}',
  '[]'::jsonb,
    jsonb_build_array('subject:9702', 'paper:p' || topic.paper::text, 'wave:${batchConfig.batchId}', 'question_text_foundation'),
...
WITH input AS (
  ...
  SELECT
    ...
    '[]'::jsonb,
    jsonb_build_object(
      'summary', '9702 question text foundation row; detailed scoring taxonomy not claimed.',
...
WITH manifest AS (
  ...
  SELECT qb.storage_key, qb.q_number, COUNT(*)::int AS duplicate_rows
  FROM public.question_bank qb
  JOIN manifest
...
SELECT json_build_object(
  'schema_version', '9702_${batchConfig.manifestSlug}_db_coverage_gate_v1',
  'status', CASE WHEN (
    ...
    AND COALESCE((SELECT SUM(duplicate_rows)::int FROM duplicate_identities), 0) = 0
  ...
  'metrics', json_build_object(
    ...
    'duplicate_storage_key_q_number_rows', COALESCE((SELECT SUM(duplicate_rows)::int FROM duplicate_identities), 0),
...
SELECT json_build_object(
  'schema_version', '9702_${batchConfig.manifestSlug}_production_search_gate_v1',
  'status', CASE WHEN (
...
SELECT json_build_object(
  'schema_version', '9702_${batchConfig.manifestSlug}_production_read_model_gate_v1',
  'status', CASE WHEN (
...
SELECT json_build_object(
  'schema_version', '9702_${batchConfig.manifestSlug}_production_rag_consumption_gate_v1',
  'status', CASE WHEN (
    COUNT(id) = COUNT(*)
...
function finalArtifactPaths({ generatedOn, reportsDir, batchConfig }) {
  const prefix = `${reportsDir}/${generatedOn}-9702-${batchConfig.reportSlug}`;
  return {
...
function buildShardProductionReport({ generatedOn, shardId, rows, aggregate, paths, batchConfig }) {
  ...
  return {
    schema_version: `9702_${batchConfig.manifestSlug}_shard_production_ready_closeout_v1`,
    generated_on: generatedOn,
    subject_code: '9702',
    wave: batchConfig.batchId,
...
function renderShardMarkdown(report) {
  return [
    `# 9702 ${report.shard_id} ${report.wave_label} production-ready closeout`,
    '',
    ...
    '- The classifier is a non-released question-text-foundation row-surface classifier; detailed 9702 scoring taxonomy is not claimed.',
...
function renderAggregateMarkdown(aggregate, paths, batchConfig) {
  return [
    `# 9702 ${batchConfig.label} production-ready closeout`,
    '',
    ...
    aggregate.status === 'pass'
      ? `9702 ${batchConfig.label} batch is production-ready for the requested row-level question text foundation surface.`
      : `9702 ${batchConfig.label} batch is not production-ready; production gate blockers remain.`,
    '',
...
function writeShardCloseouts({ generatedOn, reportsDir, readyRows, aggregate, paths, batchConfig }) {
  ...
  for (const [shardId, shardRows] of byShard.entries()) {
    ...
    const jsonPath = `${reportsDir}/${generatedOn}-${kebab}-shard-production-ready-closeout.json`;
    const markdownPath = `${reportsDir}/${generatedOn}-${kebab}-shard-production-ready-closeout.md`;
...
function updateReportsIndex({ generatedOn, indexPath, readyManifestPath, paths, batchConfig, aggregate }) {
  ...
  const entry = [
    `- \`${generatedOn}-9702-${batchConfig.reportSlug}-production-ready-closeout.md\` and \`${generatedOn}-9702-${batchConfig.reportSlug}-production-ready-gate.json\` - 9702 ${batchConfig.label} production-ready closeout for the requested ${aggregate.scope.shards} shards / ${aggregate.scope.rows} rows: DB/question_bank registry coverage ${aggregate.scope.rows}/${aggregate.scope.rows}, production search/read-model/RAG gates all consume \`question_plain_text_v2.normalized_plain_text\`, text-only ready ${aggregate.summary.text_only_ready_rows}, image-context required ${aggregate.summary.image_context_required_rows}, blockers ${aggregate.summary.blockers}, and \`production_ready_claimed=${aggregate.production_ready_claimed}\`.`,
    `- \`${readyManifestPath}\` - machine-readable 9702 ${batchConfig.label} production surface manifest linking source/crop/v2/authority/local-consumption evidence to production DB/search/read-model/RAG rows.`,
  ...
  if (current.includes(`${generatedOn}-9702-${batchConfig.reportSlug}-production-ready-closeout.md`)) {
    return;
  }
  fs.writeFileSync(resolved, current.replace(marker, `${marker}${entry}`), 'utf8');
}

async function run9702ProductionBatch({ options, batchId, evidenceGate }) {
  const batchConfig = build9702ProductionBatchConfig(batchId, {
  ...
  if (options.applyDb) {
    ...
    gates = {
      dbCoverage: defaultGateBlocked(`9702_${batchConfig.manifestSlug}_db_coverage_gate_v1`, readyRows.length, 'apply_db_not_requested', generatedOn),
      searchGate: defaultGateBlocked(`9702_${batchConfig.manifestSlug}_production_search_gate_v1`, readyRows.length, 'apply_db_not_requested', generatedOn),
      readModelGate: defaultGateBlocked(`9702_${batchConfig.manifestSlug}_production_read_model_gate_v1`, readyRows.length, 'apply_db_not_requested', generatedOn),
      ragGate: defaultGateBlocked(`9702_${batchConfig.manifestSlug}_production_rag_consumption_gate_v1`, readyRows.length, 'apply_db_not_requested', generatedOn),
    };
  }
  ...
  const finalReadyManifest = buildReadyManifest({
    generatedOn,
    readyRows,
    artifacts,
    batchConfig,
    productionReadyClaimed: aggregate.production_ready_claimed,
  });
  writeJson(batchConfig.readyManifest, finalReadyManifest);
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const evidenceGate = readJson(options.evidenceGate);
  const generatedOn = options.generatedOn ?? evidenceGate.generated_on ?? FALLBACK_GENERATED_ON;
  const batchPlan = build9702ProductionBatchPlan({
    evidenceGate,
    generatedOn,
    reportsDir: options.reportsDir,
  });
  const planPath = `data/manifests/9702_production_batch_plan_${generatedOnToken(generatedOn)}_manifest_v1.json`;
  writeJson(planPath, batchPlan);
  writeStdoutLine(`9702_phase7_batch_plan=${planPath}`);
  writeStdoutLine(`9702_phase7_batch_plan_status=${batchPlan.status}`);
  writeStdoutLine(`9702_phase7_batch_plan_batches=${batchPlan.summary.batches}`);
  writeStdoutLine(`9702_phase7_batch_plan_rows=${batchPlan.summary.selected_rows}`);
  writeStdoutLine(`9702_phase7_batch_plan_blockers=${batchPlan.summary.blockers}`);

  if (batchPlan.status !== 'ready') {
    return 1;
  }
  if (options.planOnly) {
    return 0;
  ...
  const selectedBatchIds = options.batchId === 'all'
    ? batchPlan.batches.map((batch) => batch.batch_id)
    : [normalizeBatchId(options.batchId)];
  const aggregates = [];
  for (const batchId of selectedBatchIds) {
    aggregates.push(await run9702ProductionBatch({
      options,
      batchId,
      evidenceGate,
    }));
  ...
  const passCount = aggregates.filter((aggregate) => aggregate.status === 'pass').length;
  writeStdoutLine(`9702_phase7_selected_batches=${aggregates.length}`);
  writeStdoutLine(`9702_phase7_passed_batches=${passCount}`);
  writeStdoutLine(`9702_phase7_blocked_batches=${aggregates.length - passCount}`);
  return passCount === aggregates.length ? 0 : 1;
}
