...
const DEFAULTS = Object.freeze({
  ...
  jsonOut: 'docs/reports/2026-06-05-9231-question-plain-text-v2-consumption.json',
  markdownOut: 'docs/reports/2026-06-05-9231-question-plain-text-v2-consumption-gate.md',
});
...
function printUsage() {
  console.log([
    'Usage: node scripts/learning/run_9231_question_plain_text_v2_consumption_gate.js',
...
export function parse9231QuestionPlainTextV2ConsumptionArgs(argv = process.argv.slice(2)) {
  const options = {
    ...DEFAULTS,
    help: false,
  };
...
export function run9231QuestionPlainTextV2ConsumptionGate({
...
export function main(argv = process.argv.slice(2)) {
  const options = parse9231QuestionPlainTextV2ConsumptionArgs(argv);
  if (options.help) {
    printUsage();
    return null;
  }

  const result = run9231QuestionPlainTextV2ConsumptionGate(options);
  ...
  if (result.status !== 'pass') {
    throw new Error('9231 question_plain_text_v2 consumption gate failed.');
  }
  return result;
}
