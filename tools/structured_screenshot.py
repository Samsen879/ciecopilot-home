...
def trim_whitespace(img, padding=8, threshold=12):
...
def estimate_left_margin(doc, max_offset):
    xs = []
    for page in doc:
        for line in extract_lines(page):
            text = line["text"]
    ...
    xs.sort()
    baseline = xs[0]
    return baseline, baseline + max_offset


def find_question_anchors(doc, max_offset=18):
    baseline_x, max_x = estimate_left_margin(doc, max_offset)
    ...
    # Remove duplicate anchors with same num/page close together.
    filtered = []
    for anchor in anchors:
        if not filtered:
            ...
            continue
        prev = filtered[-1]
        if anchor["page"] == prev["page"] and abs(anchor["y0"] - prev["y0"]) < 5:
            continue
        filtered.append(anchor)
...
def build_question_segments(doc, anchors, index, top_pad=2, bottom_pad=2):
    ...
    if index + 1 < len(anchors):
        ...
        segments.append((next_anchor["page"], fitz.Rect(0, next_header, next_page_rect.width, bottom_y)))
        return segments
...
def main():
    parser = argparse.ArgumentParser(description="Structured question screenshot generator")
