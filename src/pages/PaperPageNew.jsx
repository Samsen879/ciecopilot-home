  ...
  const loadFromDatabase = async () => {
    try {
      ...
      if (papersError || !papers) {
        // 尝试使用name字段匹配
        const { data: papersByName, error: papersByNameError } = await db
          .from('papers')
          .select('*')
          .eq('subject_id', subjects.id)
          .eq('name', paper);

        if (papersByNameError || !papersByName || papersByName.length === 0) {
          throw new Error(`Paper ${paper} not found for subject ${subject}`);
        }
        papers = papersByName[0];
      }
      ...
      const transformedTopics = topics.map(topic => ({
        id: normalizeTopicId(topic.name),
        name: topic.name,
        description: topic.description || `Master ${topic.name} with comprehensive learning materials.`,
        cardCount: 0, // 数据库中暂未存储此信息
        totalPoints: 0 // 数据库中暂未存储此信息
      }));

      return {
        success: true,
        data: {
          name: papers.name || papers.description || paperNames[paper] || `Paper ${paper}`,
          description: papers.description || `Learn ${subjects.name} ${papers.name} with comprehensive study materials.`,
