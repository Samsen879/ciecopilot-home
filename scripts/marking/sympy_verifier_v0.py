...
def verify_equivalence(student_expr: str, expected_expr: str, variable: str = "x") -> dict[str, Any]:
    try:
        student = _normalize_expr(student_expr)
        expected = _normalize_expr(expected_expr)
    except Exception as exc:
        return {"status": "parse_fail", "equivalent": False, "reason": str(exc)}
    ...
    if HAS_SYMPY:
        try:
            ...
            return {"status": "parse_fail", "equivalent": False, "reason": f"sympy_parse_fail: {exc}"}
    ...
    try:
        ...
        return {"status": "parse_fail", "equivalent": False, "reason": f"sampling_parse_fail: {exc}"}
...
def run_from_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for idx, item in enumerate(items):
        if not isinstance(item, dict):
            result = {"item_id": f"i{idx+1}", "status": "parse_fail", "equivalent": False, "reason": "invalid_item"}
        ...
        verdict = verify_equivalence(student_expr, expected_expr, variable=variable)
