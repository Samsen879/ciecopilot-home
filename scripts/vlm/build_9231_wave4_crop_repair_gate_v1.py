#!/usr/bin/env python3
"""Build local-only render/crop evidence for 9231 wave4 crop repair shards."""
...
SCHEMA_VERSION = "9231_wave4_crop_repair_gate_v1"
CROP_MANIFEST_SCHEMA_VERSION = "9231_wave4_crop_manifest_v1"
DEFAULT_WAVE4_REPAIR_SHARD_IDS = (
    "9231_p2_w23_standard_001",
    "9231_p2_w24_standard_001",
    "9231_p4_s21_standard_001",
    "9231_p4_s22_standard_001",
    "9231_p4_s23_standard_001",
    "9231_p4_w20_standard_001",
    "9231_p4_w21_standard_001",
    "9231_p4_w22_standard_001",
    "9231_p4_w23_standard_001",
    "9231_p4_w24_standard_001",
)
SELECTION_STRATEGY = (
    {
        "bucket": "remaining clean crop/render-incomplete Paper 2 and Paper 4 shards after wave3",
        "shard_ids": DEFAULT_WAVE4_REPAIR_SHARD_IDS,
    },
)
...
def _replace_wave4_strings(value: Any) -> Any:
    if isinstance(value, str):
        return (
            value.replace("local_pilot_crop_render", "local_wave4_crop_render")
            .replace("pilot_crop_render", "wave4_crop_render")
            .replace("9231 pilot shard", "9231 wave4 shard")
            .replace("9231 pilot shards", "9231 wave4 shards")
            .replace("representative 9231 pilot", "representative 9231 wave4")
            .replace("pilot page-chain", "wave4 page-chain")
        )
    if isinstance(value, list):
        return [_replace_wave4_strings(item) for item in value]
    if isinstance(value, dict):
        return {key: _replace_wave4_strings(item) for key, item in value.items()}
    return value


def _wave4_crop_manifest(crop_manifest: dict[str, Any], *, generated_on: str) -> dict[str, Any]:
    updated = _replace_wave4_strings(crop_manifest)
    updated["schema_version"] = CROP_MANIFEST_SCHEMA_VERSION
    updated["manifest_id"] = f"9231_wave4_crop_repair_{generated_on.replace('-', '_')}_crop_manifest_v1"
    scope = dict(updated.get("scope") or {})
    scope.update(
        {
            "stage": "9231 wave4 crop repair render/crop gate",
            "source_scope_label": "10 clean 9231 wave4 input manifests",
            "surface_update": "wave4 page-chain surface manifests receive local crop/render references only",
...
def _wave4_surface_record(record: dict[str, Any], *, generated_on: str) -> dict[str, Any]:
    payload = _replace_wave4_strings(record["payload"])
    pilot_gate = payload.pop("pilot_crop_gate", {})
    payload["wave4_crop_gate"] = {
        **pilot_gate,
    ...
    for item in payload.get("items") or []:
        if isinstance(item, dict):
            item["surface_evidence_status"] = "local_wave4_crop_render_complete_pending_visual_review"
            item["page_chain_surface_status"] = "wave4_crop_render_complete_pending_visual_review"
    return {
        **record,
        "payload": payload,
    }
...
def _wave4_report(
    report: dict[str, Any],
    *,
    wave4_shard_ids: list[str],
    generated_on: str,
    source_cleanliness_risks: dict[str, Any],
) -> dict[str, Any]:
    updated = _replace_wave4_strings(report)
    ...
    summary["wave4_shards"] = summary.pop("pilot_shards", len(wave4_shard_ids))
    updated["summary"] = summary
    updated.pop("pilot_shard_ids", None)
    updated["wave4_shard_ids"] = wave4_shard_ids
...
def render_9231_wave4_crop_repair_gate_markdown(
    report: dict[str, Any],
    *,
    ...
    lines = [
        ...
        (
            ...
            if report["gate_status"] == "wave4_crop_render_complete_pending_visual_review"
            else "Conclusion: 9231 wave4 crop repair gate is blocked and needs local locator/crop fixes before visual review."
        ),
        "",
        "## Selection Strategy",
        "",
    ...
    lines.extend(
        [
            ...
            "| metric | value |",
            "| --- | ---: |",
            f"| wave4 shards | {summary['wave4_shards']} |",
...
def build_9231_wave4_crop_repair_gate(
    *,
    wave4_shard_ids: list[str] | tuple[str, ...] = DEFAULT_WAVE4_REPAIR_SHARD_IDS,
    manifest_root: str | Path = "data/manifests",
    output_root: str | Path = "data/crops/9231-crop-repair-wave4",
    ...
    wave4_ids = [str(value) for value in wave4_shard_ids]
    result = build_9231_pilot_shard_crop_gate(
        pilot_shard_ids=wave4_ids,
    ...
    updated_surfaces = [
        _wave4_surface_record(record, generated_on=generated)
        for record in result["updated_surface_manifests"]
    ]
    crop_manifest = _wave4_crop_manifest(result["crop_manifest"], generated_on=generated)
    return {
        "crop_manifest": crop_manifest,
        "report": _wave4_report(
            result["report"],
            wave4_shard_ids=wave4_ids,
            generated_on=generated,
...
def write_9231_wave4_crop_repair_gate_outputs(
    result: dict[str, Any],
    *,
    crop_manifest_path: str | Path,
    ...
    report_md_path.write_text(
        render_9231_wave4_crop_repair_gate_markdown(
            result["report"],
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build local-only 9231 wave4 crop repair gate artifacts.")
    parser.add_argument("--wave4-shard-id", action="append", default=[])
    parser.add_argument("--manifest-root", type=Path, default=Path("data/manifests"))
    parser.add_argument("--output-root", type=Path, default=Path("data/crops/9231-crop-repair-wave4"))
    parser.add_argument(
        "--crop-manifest",
        type=Path,
        default=Path("data/manifests/9231_wave4_crop_repair_2026_06_05_crop_manifest_v1.json"),
    )
    parser.add_argument(
        "--report-json",
        type=Path,
        default=Path("docs/reports/2026-06-05-9231-wave4-crop-repair-gate.json"),
    )
    parser.add_argument(
        "--report-md",
        type=Path,
        default=Path("docs/reports/2026-06-05-9231-wave4-crop-repair-gate.md"),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    wave4_ids = args.wave4_shard_id or list(DEFAULT_WAVE4_REPAIR_SHARD_IDS)
    result = build_9231_wave4_crop_repair_gate(
        wave4_shard_ids=wave4_ids,
    ...
    write_9231_wave4_crop_repair_gate_outputs(
        result,
    ...
    return 0 if result["report"]["gate_status"] == "wave4_crop_render_complete_pending_visual_review" else 1
